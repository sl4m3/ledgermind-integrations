"""Strict object-facet resolution and retrieval contracts v2.

The module is deliberately not imported from ``ledgermind_protocol.__init__``
yet. D3 owns public package exports; keeping this module standalone lets the
new boundary be tested before the IPC cutover.
"""
...
class MentionResolutionInput(ProtocolModel):
    ...
    def validate_mention(self) -> MentionResolutionInput:
        ...
        if _normalize_for_comparison(self.surface_text) != self.normalized_text:
            raise ValueError("normalized_text does not match the v2 name normalizer")
...
class LedgermindContextV1(ProtocolModel):
    ...
    @model_validator(mode="after")
    def validate_context_extension(self) -> LedgermindContextV1:
...
class RawRoundContextExtension(ProtocolModel):
    ledgermind_context_v1: LedgermindContextV1


def validate_raw_round_extensions(
    extensions: Mapping[str, Any] | RawRoundContextExtension,
) -> RawRoundContextExtension | None:
    """Validate ``ledgermind_context_v1`` without changing RawRound v2."""

    if isinstance(extensions, RawRoundContextExtension):
        return extensions
    if not isinstance(extensions, Mapping):
        raise TypeError("RawRound extensions must be an object")
    if "ledgermind_context_v1" not in extensions:
        return None
    context = extensions["ledgermind_context_v1"]
    return RawRoundContextExtension.model_validate({"ledgermind_context_v1": context})
